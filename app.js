...
// Uses Formspree (https://formspree.io) for real email delivery.
// Steps to activate:
//   1. Go to https://formspree.io and sign up (free — 50 submissions/month).
//   2. Create a new form, set the recipient email to lokeshboddapu1234@gmail.com.
//   3. Copy your form's endpoint ID (looks like: xpwzabcd) from the dashboard.
//   4. Replace YOUR_FORMSPREE_ID below with that ID.
const FORMSPREE_ENDPOINT = 'https://formspree.io/f/xojzbvgz';

function contact(){ const form=q('#contact-form'); if(!form) return; const emailRe=/^[^\s@]+@[^\s@]+\.[^\s@]+$/; let note=null; const show=(msg,type='info')=>{ if(!note){ note=document.createElement('div'); document.body.appendChild(note); } note.className=`notification notification--${type}`; note.setAttribute('role','status'); note.setAttribute('aria-live','polite'); note.innerHTML=`<div class="notification-content"><span class="notification-message">${msg}</span><button class="notification-close" type="button" aria-label="Close">×</button></div>`; requestAnimationFrame(()=>note.classList.add('notification--visible')); clearTimeout(show._t); show._t=setTimeout(()=>{ hide(); },4800); }; const hide=()=>{ if(note){ note.classList.remove('notification--visible'); setTimeout(()=>note&&note.remove(),280); note=null; } }; document.addEventListener('click',e=>{ if(e.target.closest('.notification-close')) hide(); }); form.addEventListener('submit',async e=>{ e.preventDefault(); const fd=new FormData(form); const data={ name:fd.get('name')?.trim(), email:fd.get('email')?.trim(), subject:fd.get('subject')?.trim(), message:fd.get('message')?.trim() }; if(Object.values(data).some(v=>!v)){ show('Please fill in all fields.','error'); return; } if(!emailRe.test(data.email)){ show('Please enter a valid email address.','error'); return; } const btn=form.querySelector('button[type="submit"]'); const orig=btn.textContent; btn.textContent='Sending...'; btn.disabled=true; btn.style.opacity='0.7'; try{ const res=await fetch(FORMSPREE_ENDPOINT,{ method:'POST', headers:{'Accept':'application/json','Content-Type':'application/json'}, body:JSON.stringify({ name:data.name, email:data.email, subject:data.subject, message:data.message }) }); if(res.ok){ show('Thank you for your message! I\'ll get back to you soon.','success'); form.reset(); } else { const body=await res.json().catch(()=>({})); const errMsg=body?.errors?.map(er=>er.message).join(', ')||'Something went wrong. Please try again.'; show(errMsg,'error'); } } catch(_){ show('Network error — please check your connection and try again.','error'); } finally{ btn.textContent=orig; btn.disabled=false; btn.style.opacity='1'; } }); }
